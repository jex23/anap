...
import 'graduate_apply_now_form.dart'; // Import the new form

class GraduateJobDetails extends StatelessWidget {
  final Map<String, dynamic> job;

  GraduateJobDetails({required this.job});
        ...
        child: ListView(
          children: [
            ...
            Row(
              ...
              children: [
                _infoCard(
                    'Payment Style', job['salaryPaymentStyle'] ?? 'Unknown'),
                _infoCard(
                  'Expiry',
                  job['expiry'] ?? 'No Expiry Date',
                ),
              ],
            ),
            SizedBox(height: 16),
            ...
            ElevatedButton(
              onPressed: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => GraduateApplyNowForm(
                      job: job,
                      user: FirebaseAuth.instance.currentUser,
                      employerId: job['employerId'], // Pass the employerId
                    ),
                  ),
                );
              },
