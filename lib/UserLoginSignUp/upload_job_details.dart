...
import 'userHomepage.dart'; // Import HomePage
...
class _UploadResumeScreenState extends State<UploadResumeScreen> {
  ...
  Future<void> _uploadResume() async {
    ...
    if (_resumeFile == null || selectedCategories.isEmpty) return;
    ...
    try {
      final ref = FirebaseStorage.instance
          .ref()
          .child('resumes')
          .child('${user.uid}_resume.${_resumeFile!.path.split('.').last}');
      await ref.putFile(_resumeFile!);

      final downloadUrl = await ref.getDownloadURL();

      await FirebaseFirestore.instance.collection('Applicants_Resume').doc(user.uid).set({
  ...
  void _addExperience() {
    if (_experienceController.text.isNotEmpty &&
      ...
      setState(() {
        ...
        _startDate = null;
        _endDate = null;
        _startDateText = "Select Start Date";
